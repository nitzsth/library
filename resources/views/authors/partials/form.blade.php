<form method="POST"
  ...
  <div class="box-body">
    ...
    <div class="form-group {{ $errors->has('death') ? 'has-error' : '' }}">
      ...
      <input type="integer"
             class="form-control"
             placeholder="Enter Author's DOD"
             value="{{ old('death') ?? $author->death }}"
             name="death">
      @if ($errors->has('death'))
        <span class="help-block">
        ...
        <select name='category_id[]' multiple class="form-control">
          @foreach($categories as $category)
            <option value="{{ $category->id }}"
                    @if(in_array($category->id, $author->categories->pluck('id')->toArray())) selected @endif >
              {{ $category->name }}
            </option>
          @endforeach
        </select>
    ...
    <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
      ...
      <textarea class="form-control"
                ...
                name="description"
                required>{{ old('description') ?? $author->description }}</textarea>
